...
void NRF24L01_Init(void)
{
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE);	 //使能PB,D,G端口时钟

	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13;	
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_Init(GPIOC, &GPIO_InitStructure);//初始化指定IO CS PC13
	
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14; 	//PC14
	GPIO_Init(GPIOC, &GPIO_InitStructure);//初始化指定IO CE

	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_15;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PC15 输入 IRQ
...
u8 NRF24L01_Read_Buf(u8 reg, u8 *pBuf, u8 len)
{
	...
	for (u8_ctr = 0; u8_ctr < len; u8_ctr++)pBuf[u8_ctr] = SPI2_ReadWriteByte(0XFF); //读出数据//0xff只是为了发起时钟。
