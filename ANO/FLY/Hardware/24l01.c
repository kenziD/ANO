...
void NRF24L01_Init(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);//初始化指定IO CS PA4
	
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_2;
	GPIO_Init(GPIOB, &GPIO_InitStructure);//初始化指定IO CE PB2
	
	GPIO_InitStructure.GPIO_Pin  = GPIO_Pin_0;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPD; //PB0 输入  IRQ
	GPIO_Init(GPIOB, &GPIO_InitStructure);

	GPIO_ResetBits(GPIOB, GPIO_Pin_0 | GPIO_Pin_2 ); //PB0,2上拉
	...
	NRF24L01_CE = 0; 			//使能24L01 进入待机模式 或者说是掉电模式，掉电模式就是各功能关闭 因为数据手册是要求在写SPI寄存器之前要进入掉电模式或者待机模式
...
//*pBuf:数据指针
//len:数据长度
/*返回值,此次读到的状态寄存器值 发射缓冲区访问函数：主要用来把数组里的数放到发射 FIFO缓冲区中。基本思路就是通过
WRITE_REG命令把数据存到发射FIFO（WR_TX_PLOAD）中去。 */
...
void NRF24L01_TX_Mode(void)
{
	...
	NRF24L01_Write_Reg(WRITE_REG_NRF + RF_SETUP, 0x0f); //设置TX发射参数,0db增益,2Mbps,低噪声增益开启
	NRF24L01_Write_Reg(WRITE_REG_NRF + NRF_CONFIG, 0x0e); //配置基本工作模式的参数;PWR_UP,EN_CRC,16BIT_CRC,发射模式,开启所有中断
	NRF24L01_CE = 1; //CE为高,10us后启动发送
}
void NRF24L01_Mode_Config(u8 mode)
{
	NRF24L01_CE = 0;
	NRF24L01_Write_Buf(WRITE_REG_NRF + RX_ADDR_P0, (u8*)RX_ADDRESS, RX_ADR_WIDTH); //设置TX节点地址,主要为了使能ACK。对于发送端：接收响应的时候要知道接哪个地址的响应。对于接收端：
	NRF24L01_Write_Reg(WRITE_REG_NRF + EN_AA, 0x01);  //使能通道0的自动应答
	NRF24L01_Write_Reg(WRITE_REG_NRF + EN_RXADDR, 0x01); //使能通道0的接收地址
	NRF24L01_Write_Reg(WRITE_REG_NRF + RF_CH, 40);    //设置RF通道为40
	NRF24L01_Write_Reg(WRITE_REG_NRF + RF_SETUP, 0x0f); //设置TX发射参数,0db增益,2Mbps,低噪声增益开启
